      ...
      <form className='flex'>
        ...
        <div className="container">
          ...
          <div className="cards">
            { cards.filter(
            (card)=>{
              return card.titre.includes(searchterm) ||card.niveau.includes(searchterm)||(card.timestamp.toLocaleDateString()).includes(searchterm)
            }).map((card) =>(
                ...
                {card.img!=null &&<img className="images" src={card.img} alt=""/>}
                   ...
                   <h3 className="texts">{card.niveau}</h3>  
                  <p className="texts">{card.discription}</p>
                  <h6 className="profs">{card.nom}</h6>
                  <h3 className="texts">{card.timestamp}</h3> 
