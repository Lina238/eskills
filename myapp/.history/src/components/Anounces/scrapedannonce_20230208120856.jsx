...
function ScrappedAnounces () {
  ...
  const handlechange=(data,card)=>{
    if(data=="checked"){
      if(checked==true){
        const uploadData = new FormData();
        uploadData.append('titre', card.titre);
        uploadData.append('niveau', card.niveau);
        uploadData.append('discription', card.discription);
        uploadData.append('img',card.img);
        uploadData.append('image1',card.image1)
        uploadData.append(' mode', card.mode);
        {card.img!=''||card.img!=null&&
        <img className="images" src={card.img}/>}
        {!card.image&&
        <img className="images" src={card.image1}/>}
        uploadData.append('nom',card.nom);
        uploadData.append('prix', card.prix);
        uploadData.append('lieu', card.lieu);
        // fetch('http://127.0.0.1:8000/core/annonces/'+card.id, {
        //   method: 'DELETE',
          ...
          <div className="cards">
            {cards.map((card) =>(
                <div key={card.id} className="card">
                  ...
                  <input type="checkbox" value={checked} onChange={()=>handlechange('checked',card) }/> ajouter
                  <h6 className="prices">{card.prix}</h6>
                  <Link to ="/DetailsAnounce"><img className="images" src={card.image}/></Link>
